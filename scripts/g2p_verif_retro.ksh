...
goodarch=/meso/save/Alicia.Bentley/good-usa-dlyprcp/$yyyy
goodgaugefile=good-usa-dlyprcp-$day
exedir=/meso/save/Alicia.Bentley/pcpverif/nextjif/exec
exedirx=/meso/save/Alicia.Bentley/pcpverif/nextjif/sorc/verif_precip_g2pt.fd

archive=/meso/save/Alicia.Bentley/verf.grid2gauge.dat

if [ ! -d $archive ]; then mkdir -p $archive; fi

wrkdir=/stmpp1/Alicia.Bentley/g2p_verif
if [ -d $wrkdir ]; then
   rm -f $wrkdir/*
else
  mkdir -p $wrkdir
...
pcpanldir=/ptmpp1/Alicia.Bentley/verf.rtma/prod_nam_pcpn_anal.$day
cp $pcpanldir/ST2.${day}12.24h.gz .
pcpanldirx=/ptmpp1/Alicia.Bentley/verf.rtma/para_nam_pcpn_anal.$day
cp $pcpanldirx/ST2x.${day}12.24h.gz .
cp $pcpanldirx/ST4.${day}12.24h.gz .

gunzip *.gz

verfdat=/ptmpp1/Alicia.Bentley/verf.rtma/precip.$daym1
